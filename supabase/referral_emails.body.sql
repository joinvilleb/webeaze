...
begin
  ...
  -- What was promised when this referral was made, not what the programme pays today. A holiday
  -- boost must still pay out at the boosted rate weeks later.
  amt        := coalesce(new.reward_amount, 50)::text;
  ...
  -- ── 0. Client logged it themselves from the portal ───────────────────────
  -- Fires on INSERT only (old is null), so it never re-sends when the admin edits the row later.
  if old is null and new.status = 'Referred' then
    inner_html :=
      '<p style="margin:0 0 16px;">Hi ' || first_name || ',</p>' ||
      '<p style="margin:0 0 16px;">Thanks for sending ' || referred || ' our way. We have logged it against your account.</p>' ||
      '<p style="margin:0 0 16px;">From here we watch for them signing up. If they do, we will email you again to confirm your referral is being tracked, and once more when your reward is ready.</p>' ||
      '<p style="margin:0 0 16px;">If they get in touch, it helps enormously if they mention your name.</p>' ||
      '<p style="margin:0 0 20px;">You can see this and every other referral you have made in your <a href="https://portal.webeaze.io/#referrals" style="color:#7851a9;font-weight:600;text-decoration:none;">portal</a>.</p>';
    email_body := json_build_object(
      'from', 'WebEaze <support@webeaze.io>',
      'to', array[referrer_email],
      'subject', 'We got your referral',
  ...
  if new.status = 'Pending' and (old.status is null or old.status is distinct from 'Pending') then
    inner_html :=
      ...
      '<p style="margin:0 0 16px;">Once they have been with us for 7 days your $' || amt || ' reward is confirmed, and we will email you to arrange the payment.</p>' ||
      '<p style="margin:0 0 20px;">Nothing for you to do in the meantime. Reply to this email if you have any questions.</p>';
    email_body := json_build_object(
      'from', 'WebEaze <support@webeaze.io>',
      'to', array[referrer_email],
  ...
  if new.status = 'Signed up' and (old.status is null or old.status is distinct from 'Signed up') then
    inner_html :=
      ...
      '<p style="margin:0 0 16px;">We''ll send you <strong>$' || amt || '</strong> in the form of an electronic payment to your preferred method.</p>' ||
    ...
    email_body := json_build_object(
      'from', 'WebEaze <support@webeaze.io>',
      'to', array[referrer_email],
      'subject', 'Your $' || amt || ' referral reward is ready',
      'html', shell_top || inner_html || shell_bottom
    )::jsonb;
    perform net.http_post(url := 'https://webeaze-mailer.webeaze-web-design.workers.dev'::text, body := email_body);
  end if;
  ...
  if new.reward_paid = true and (old.reward_paid is null or old.reward_paid = false) then
    inner_html :=
      '<p style="margin:0 0 16px;">Hi ' || first_name || ',</p>' ||
      '<p style="margin:0 0 16px;">Your $' || amt || ' referral reward for ' || referred || ' has been sent to the payment details you gave us.</p>' ||
    ...
    email_body := json_build_object(
      'from', 'WebEaze <support@webeaze.io>',
      'to', array[referrer_email],
      'subject', 'Your $' || amt || ' reward has been sent',
      'html', shell_top || inner_html || shell_bottom
    )::jsonb;
    perform net.http_post(url := 'https://webeaze-mailer.webeaze-web-design.workers.dev'::text, body := email_body);
  end if;
