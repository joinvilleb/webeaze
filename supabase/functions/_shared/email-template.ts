...
async function registry(): Promise<Record<string, { subject: string; slots: Slots }>> {
  ...
  try {
    const res = await fetch(REGISTRY_URL, { headers: { 'Cache-Control': 'no-cache' } });
    if (res.ok) {
...
export async function emailCopy(
  ...
  let override: { subject?: string | null; slots?: Slots | null } | null = null;
  try {
    const { data } = await service.from('email_overrides').select('subject, slots').eq('key', key).maybeSingle();
    override = data || null;
  } catch (e) {
