// Is this inquiry a pitch dressed as a customer?
//
// WAS COPIED, NOW SHARED. This lived inline in lead-digest with a comment saying an edge function
// and a static page share no module system, so it had to be duplicated. True for the portal, which
// is a static HTML file, but NOT true of two edge functions: form-lead needs the same verdict at the
// moment a message arrives so the score can be stored on the row. portal/index.html still carries
// its own copy and still has to be changed alongside this one.
//
// Re-run scratchpad/spamtest.js against the 20-message set after ANY change here.

// ── Suspected spam on a lead ─────────────────────────────────────────────────
// Cold B2B pitches, SEO spam and scams arrive through the same contact form as real customers.
//
// The asymmetry decides the design: a missed spam costs the owner five seconds. A real customer
// wrongly flagged costs them a JOB. So the SUPPRESSORS carry most of the weight. A genuine trade
// inquiry names a job or a property, or places it somewhere, or puts a time on it, and any one of
// those pushes the score well below the line no matter what else it contains. That is what lets a
// customer paste a Facebook link, shout in caps about a flood, say "message me on WhatsApp", or ask
// about a commercial rate for their own company, without being touched.
//
// Nothing is ever hidden or deleted on the strength of this. The portal dims the row and says why.
...
// Nobody asking a landscaper for a quote sends their own booking page. This is the single strongest
// signal of a pitch there is, and nothing in the old set looked for it: a Calendly link scored zero.
...
// Somebody selling their own time rather than asking to buy yours. A customer describes a job they
// need doing; this describes a person who needs work.
...
let hard = false;
const addHard = (n: number, reason: string) => { hard = true; add(n, reason); };
SPAM_SERVICE.lastIndex = 0;
const services = [...new Set((msg.match(SPAM_SERVICE) || []).map((s: string) => s.toLowerCase()))];
...
const hosts = (msg.match(/https?:\/\/([^\s/"'<>]+)/gi) || []).map((u: string) => u.replace(/^https?:\/\//i, '').replace(/^www\./i, '').toLowerCase());
if (hosts.some((h: string) => SPAM_SHORTENER.test(h))) addHard(5, 'a shortened link');
